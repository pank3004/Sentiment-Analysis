...
def main():
    try:
        ...
        df = load_data(data_url='https://raw.githubusercontent.com/pank3004/Datasetss/refs/heads/main/IMDB_subset.csv')
        # s3 = s3_connection.s3_operations("bucket-name", "accesskey", "secretkey")
        # df = s3.fetch_file_from_s3("IMDB_subset.csv")



        final_df = preprocess_data(df)
        logging.info('train test spliting....')
        train_data, test_data = train_test_split(final_df, test_size=test_size, random_state=42)
        save_data(train_data, test_data, data_path='./data')
        logging.info('data train and test data saved at data folder')
    except Exception as e:
                raise MyException(e,sys)
